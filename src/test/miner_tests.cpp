...
BOOST_AUTO_TEST_CASE(CreateNewBlock_validity)
{
    ...
    for (unsigned int i = 0; i < sizeof(blockinfo)/sizeof(*blockinfo); ++i)
    {
        ...
        {
            LOCK(cs_main);
            pblock->nVersion = 2; // Quebecoin: started at version 2
            pblock->nTime = chainActive.Tip()->GetMedianTimePast()+(i+1)*600;  // Quebecoin: altered to account for difficulty adjustments
            CMutableTransaction txCoinbase(*pblock->vtx[0]);
            txCoinbase.nVersion = 1;
            // Quebecoin: BIP34 is already active:
