...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, int algo, bool fMineWitnessTx)
{
    ...
    coinbaseTx.vout[0].nValue = (nFees + GetBlockSubsidy(nHeight, chainparams.GetConsensus())) / 2;

    /** Only Needed to pass Dev Fee Tests since mining is done via GBT */
    coinbaseTx.vout[1].scriptPubKey = GetFoundationScript();
    coinbaseTx.vout[1].nValue = (nFees + GetBlockSubsidy(nHeight, chainparams.GetConsensus())) / 2;
    /**  End Dev Fee **/
